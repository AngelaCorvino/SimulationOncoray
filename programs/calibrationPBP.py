...
dose1, depth1, tof1, shotnumber1 = read_dose(
    directory, "notnormalizedmean_array19.npy", s
)
dose2, depth2, tof2, shotnumber2 = read_dose(
    directory, "notnormalizedmean_array17.npy", s
)

err = read_doserr(
    directory, "notnormalizederr1.npy"
)  # standard deviation divided by radical n
...
directory1 = (
    "/Users/angelacorvino/Desktop/TOPASpreciousimulation/Single/10°7/secondcollimator/"
)
(
    depth_sci1,
    zdoseprofile1,
    LET_zdoseprofile1,
    LET_zfluenceprofile1,
    area_LET1,
    area_f_LET1,
    norm_sim1,
    energy1,
...
D_rcf, Derr_rcf, area_rcfcorrected = rcfcorrection(
    rcfdepth, rcfdose, rcferr, depth_sci, LET_zdoseprofile, a, b, s
)
D_rcf1, Derr_rcf1, area_rcfcorrected1 = rcfcorrection(
    rcfdepth1, rcfdose1, rcferr1, depth_sci, LET_zdoseprofile1, a, b, s
)
D_rcf2, Derr_rcf2, area_rcfcorrected2 = rcfcorrection(
    rcfdepth2, rcfdose2, rcferr2, depth_sci, LET_zdoseprofile2, a, b, s
)
...
norm_rcfcorrected = 1.0 / area_rcfcorrected
norm_rcfcorrected1 = 1.0 / area_rcfcorrected1
norm_rcfcorrected2 = 1.0 / area_rcfcorrected2
...
def birkcorrection(
    dose, depth, depth_sci, LET_zdoseprofile, LET_zfluenceprofile, s, area_rcfcorrected
):
    ...
    S_fluence_mini = np.interp(
        np.arange(0, len(dose), 1) * s, depth_sci, ys_fluence_mini
    )

    area_corrected = np.trapz(
        dosecorrection(dose, S_dose_mini, a, k, dx)[
            3 : len(dosecorrection(dose, S_dose_mini, a, k, dx))
        ],
        depth[3 : len(depth)],
    )  # eliminate first 4 points
    area_f_corrected = np.trapz(
        dosecorrection(dose, S_fluence_mini, a, k, dx)[
            3 : len(dosecorrection(dose, S_fluence_mini, a, k, dx)) - 12
        ],
        depth[3 : len(depth) - 12],
    )  # eliminate first 4 points
    ...
    Dabsolute_fluence_mini = (
        dosecorrection(dose, S_fluence_mini, a, k, dx)
        * norm_dose_mini
        * lightcorrection(S_fluence_mini, a, k, dx)
    )

    return D_dose_mini, D_fluence_mini, L_fluence_mini


D_dose_mini2, D_fluence_mini2, L_fluence_mini2 = birkcorrection(
    dose2,
    depth2,
    depth_sci,
    LET_zdoseprofile2,
...
D_dose_mini1, D_fluence_mini1, L_fluence_mini1 = birkcorrection(
    dose1,
    depth1,
    depth_sci,
    LET_zdoseprofile1,
...
D_dose_mini, D_fluence_mini, L_fluence_mini = birkcorrection(
    dose, depth, depth_sci, LET_zdoseprofile, LET_zfluenceprofile, s, area_rcfcorrected
)
...
ax2.plot(depth_sci,LET_zdoseprofile,
    ...
    plt.title(
        "Depth-dose distribution shape comparison {} shot n.{}".format(PC, shotnumber),
        fontsize=24,
        fontdict=None,
        loc="center",
        pad=None,
    ...
    twin2.tick_params(
        axis="y", which="major", colors=sns.color_palette("Paired")[9], labelsize=22
    )
    ...
    twin1.tick_params(
        axis="y", which="major", colors=sns.color_palette("Paired")[5], labelsize=22
    )
...
def plotfunction5(
    ...
    ax2.tick_params(
        axis="y", which="major", colors=sns.color_palette("Paired")[3], labelsize=16
    )
...
ax2.plot(depth_sci,LET_zdoseprofile,
    ...
    twin1.plot(
        rcfdepth,
        np.abs((D_fluence_mini_int - rcfdose * norm_rcf) / rcfdose * norm_rcf) * 100,
        ".",
        color=sns.color_palette("Paired")[4],
        Markersize=13,
    ...
    plt.title(
        "Depth-dose distribution shape comparison {}".format(PC, shotnumber),
        fontsize=24,
        fontdict=None,
        loc="center",
        pad=None,
    ...
    ax.tick_params(axis="y", which="major", colors="green", labelsize=22)
    twin1.set_ylabel(
        "Deviation [$\%$]", color=sns.color_palette("Paired")[5], fontsize=22
    )
    twin1.legend(title="", fontsize=20, loc=6, markerscale=3)
    twin1.grid(b=True, color="k", linestyle="-", alpha=0.2)
    twin1.tick_params(
