...
dose2, depth2, tof, shotnumber2 = read_dose(
    directory, "notnormalizedmean_array29.npy", s
)
# dose2=dose2-dose2.min()
dose, depth, tof, shotnumber = read_dose(directory, "notnormalizedmean_array34.npy", s)
dose1, depth1, tof, shotnumber1 = read_dose(
    directory, "notnormalizedmean_array9.npy", s
)
...
err2 = read_doserr(
    directory, "notnormalizederr29.npy"
...
area_sim = np.trapz(
    zdoseprofile[3 : len(zdoseprofile) - 8], depth_sci[3 : len(depth_sci) - 8]
)
...
area_sim1 = np.trapz(
    zdoseprofile1[0 : len(zdoseprofile1) - 8], depth_sci1[0 : len(depth_sci1) - 8]
)  # eliminate last 3 points
...
area_sim2 = np.trapz(
    zdoseprofile2[0 : len(zdoseprofile2) - 8], depth_sci2[0 : len(depth_sci2) - 8]
)  # eliminate last 3 points
...
D_rcf1, Derr_rcf1, area_rcfcorrected1 = rcfcorrection(
    rcfdepth, rcfdose1, rcferr1, depth_sci1, LET_zdoseprofile1, a, b, s
)
D_rcf2, Derr_rcf2, area_rcfcorrected2 = rcfcorrection(
    rcfdepth2, rcfdose2, rcferr2, depth_sci2, LET_zdoseprofile2, a, b, s
)
####################################################################################
...
def birkcorrection(
    ...
    area_corrected = np.trapz(
        dosecorrection(dose, S_a_mini, a, k, dx)[
            3 : len(dosecorrection(dose, S_a_mini, a, k, dx))
        ],
        depth[3 : len(depth)],
...
D_a_mini2, D_fluence_mini2 = birkcorrection(
    dose2,
    depth2,
    depth_sci2,
    LET_zdoseprofile2,
...
D_a_mini1, D_fluence_mini1 = birkcorrection(
    dose1,
    depth1,
    depth_sci1,
    LET_zdoseprofile1,
...
D_a_mini, D_fluence_mini = birkcorrection(
    dose, depth, depth_sci, LET_zdoseprofile, LET_zfluenceprofile, s, area_rcfcorrected
)
...
def plotfunction3(
    ...
    ax.tick_params(axis="x", which="major", labelsize=22)
    ax2.tick_params(
        axis="y", which="major", colors=sns.color_palette("Paired")[2], labelsize=22
    )
