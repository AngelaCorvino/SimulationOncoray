...
outputfile_topas = '/home/corvin22/SimulationOncoray/DoseToWater_150MeVproton_prova.csv'
...
# read in the csv-file
outputfile_topas = '/home/corvin22/SimulationOncoray/EnergyDeposit_150MeVproton_prova.csv'
header = pd.read_csv(outputfile_topas, nrows = 7)
df = pd.read_csv(outputfile_topas, comment='#', header=None)
...
plt.title('Dose in PVT(aperture diameter= 7mm, 11 PC)') # Title
...
plt.plot(np.arange(0,numberof_zbins,1)*0.2, zmeanprofile,'.-',label='{ mean value of 161 bin along x}')

#plt.plot(depthPVT, dosePVT,'.-',label='{70 MeV protons ,0cm distance}') # plots depth on x, dose on y, and uses default layout
plt.title('Dose in PVT(aperture diameter=7mm 11PC) ') # Title
