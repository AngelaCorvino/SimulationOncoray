...
outputfile_topas = '/home/corvin22/SimulationOncoray/DoseToWater_90MeVproton_PVT_12PC_SOBP.csv'
...
# read in the csv-file
outputfile_topas = '/home/corvin22/SimulationOncoray/EnergyDeposit_90MeVproton_PVT_12PC_SOBP.csv'
header = pd.read_csv(outputfile_topas, nrows = 7)
df = pd.read_csv(outputfile_topas, comment='#', header=None)
...
plt.plot(np.arange(0,numberof_xbins,1)*0.0634, xmeanprofile,'.-',label='{mean value of 170 bins along z}') # plots depth on x, dose on y, and uses default layout


#plt.plot(depthPVT, dosePVT,'.-',label='{70 MeV protons ,0cm distance}') # plots depth on x, dose on y, and uses default layout
plt.title('Dose in PVT(aperture diameter= 7mm, 12 PC)') # Title
